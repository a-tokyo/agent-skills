...
| [tribunal](skills/tribunal/) | Doer → verifier-panel → consensus delivery verification for any artifact (code slices, plans, documents, audits). An orchestrator freezes acceptance criteria before implementation, spawns a doer, then convenes a context-walled panel — Quality, Utility, and a Devil's Advocate — for independent evidence-anchored scoring, a single anonymized synthesis round, and evidence-adjudicated consensus ending in SHIP / SHIP_WITH_CAVEATS / ITERATE / BLOCK / ESCALATE, logged to a gate ledger. Platform-agnostic; degrades to sequential fresh-context sessions without subagents. A/B-benchmarked (blind-judged, seeded-defect + implement-and-verify tasks) — results in its README. |
